...
describe('Padlet board by class service functional tests', () => {
    beforeAll(async () => {
    ...
    it('should get all padlet boards by class for professor', async () => {
        const ClassId = "66eee6eaa3c7c5bfd3c2f091";
        const response = await request(server)
            .get(`/api/padlet/board/classroom/${ClassId}`)
    ...
    it('should return error for class id not found', async () => {
        ...
        const response = await request(server)
            .get(`/api/padlet/board/classroom/${nonExistentId}`)
            .set('Authorization', `Bearer ${authToken}`)
            .expect(404);
