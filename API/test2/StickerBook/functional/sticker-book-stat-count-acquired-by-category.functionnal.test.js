...
describe("Sticker book stat count acquired by category service functional tests", () => {
    beforeAll(async () => {
    ...
    it("should return stickers count and acquired count by category for the student", async () => {
        const response = await request(server)
            .get(`/api/sticker-book/stat-count-acquired-by-category/${studentId}`)
    ...
    it("should return 404 if the student does not exist", async () => {
        ...
        const response = await request(server)
            .get(`/api/sticker-book/stat-count-acquired-by-category/${nonExistentStudentId}`)
            .set("Authorization", `Bearer ${authTokenProfessor}`)
            .expect(404);
